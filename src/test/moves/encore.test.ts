...
describe("Moves - Encore", () => {
  ...
  beforeEach(() => {
    ...
    game.override
      .moveset([ Moves.ENCORE, Moves.SPLASH, Moves.DISABLE ])
  ...
  it("should force the target to use Struggle when the Encored move is disabled", async () => {
    await game.classicMode.startBattle([ Species.SNORLAX ]);
    ...
    expect(enemyPokemon.getTag(BattlerTagType.ENCORE)).toBeDefined();
    expect(enemyPokemon.getTag(BattlerTagType.DISABLED)).toBeDefined();
    expect(enemyPokemon.getLastXMoves(1)[0].move).toBe(Moves.SPLASH);
    expect(enemyPokemon.getLastXMoves(1)[0].result).toBe(MoveResult.SUCCESS);

    game.move.select(Moves.SPLASH);

    await game.phaseInterceptor.to("BerryPhase", false);
    expect(enemyPokemon.getLastXMoves(1)[0].move).toBe(Moves.STRUGGLE);
  });
});
